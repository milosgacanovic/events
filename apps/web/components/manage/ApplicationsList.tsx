    ...
    <div>
      ...
      {dialog && (
        <div role="dialog" aria-modal="true" style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.45)" }} onClick={() => setDialog(null)} />
          <div className="manage-event-card" style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 520, margin: 16, padding: 24 }}>
            <h2 style={{ margin: "0 0 8px", fontSize: "1.15rem" }}>{dialogTitles[dialog.type]}</h2>
            <p className="meta" style={{ margin: "0 0 12px" }}>Applicant: <strong>{dialog.appName}</strong></p>
            <textarea
              ...
              rows={8}
              style={{ width: "100%", padding: 10, borderRadius: 6, border: "1px solid var(--border-color, #ccc)", fontFamily: "inherit", fontSize: "0.9rem", resize: "vertical", background: "var(--input-bg, #fff)", color: "var(--text-color, #333)" }}
            />
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
              <button type="button" className="ghost-btn" onClick={() => setDialog(null)} disabled={submitting}>Cancel</button>
              <button
                type="button"
