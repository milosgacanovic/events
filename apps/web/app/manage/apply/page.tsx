...
export default function ApplyPage() {
  ...
  const [intent, setIntent] = useState("organize_events");
  ...
  }, [auth.userName]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <form className="manage-form" onSubmit={(e) => void handleSubmit(e)}>
        ...
        <div>
          ...
          <select value={intent} onChange={(e) => setIntent(e.target.value)}>
            ...
            <option value="teach_classes">I teach dance classes</option>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 4 }}>
              {practices.map((p) => (
                <label key={p.id} style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer" }}>
                  <input
                    type="checkbox"
                    checked={practiceCategoryIds.includes(p.id)}
                ...
                <div style={{ border: "1px solid var(--border)", borderRadius: 6, marginTop: 4, maxHeight: 160, overflow: "auto" }}>
                  {hostResults.map((h) => (
                    <button
                      ...
                      style={{ display: "block", width: "100%", padding: "8px 12px", textAlign: "left", border: "none", background: "transparent", cursor: "pointer" }}
                    >
                      {h.name}
                    </button>
                  ))}
