...
export function LogsContent({ tab }: { tab: "activity" | "errors" }) {
  ...
  const [errorPage, setErrorPage] = useState(() => parseInt(searchParams.get("page") ?? "1", 10) || 1);
  const [errorSearch, setErrorSearch] = useState(() => searchParams.get("q") ?? "");
  const [errorDateFrom, setErrorDateFrom] = useState(() => searchParams.get("dateFrom") ?? "");
  const [errorDateTo, setErrorDateTo] = useState(() => searchParams.get("dateTo") ?? "");
  const [errorLoading, setErrorLoading] = useState(false);
  const [errorError, setErrorError] = useState("");

  // Sync state to URL (scoped to active tab's filters)
  useEffect(() => {
    ...
    if (tab === "activity") {
      ...
      if (errorPage > 1) params.set("page", String(errorPage));
    }
    const qs = params.toString();
    const url = qs ? `${pathname}?${qs}` : pathname;
    router.replace(url, { scroll: false });
  }, [tab, activitySearch, actionFilter, targetTypeFilter, actorFilter, excludeServiceAccounts, activityDateFrom, activityDateTo, activityPage, errorSearch, errorDateFrom, errorDateTo, errorPage, pathname, router]);
  ...
  // --- Load actors for filter (only needed on activity tab) ---
  useEffect(() => {
  ...
  // Fire only the active tab's loader — the inactive route isn't mounted.
  useEffect(() => {
