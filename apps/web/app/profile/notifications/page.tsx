...
export default function NotificationsTab() {
  ...
  async function togglePauseAll(paused: boolean) {
    const token = await getToken();
    if (!token) return;
    const res = await fetch(`${apiBase}/profile/saved-searches/pause-all`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    ...
    if (res.ok) {
      setItems((cur) =>
        cur.map((i) => ({ ...i, unsubscribedAt: paused ? new Date().toISOString() : null })),
      );
    }
  }
  ...
  if (items.length === 0) return <p className="muted">{t("profile.savedSearches.empty")}</p>;
    ...
    <>
      <label className="toggle-control" style={{ marginBottom: 12 }}>
        <input
          ...
          type="checkbox"
          checked={items.every((s) => s.unsubscribedAt != null)}
        ...
        {items.map((search) => (
          <li key={search.id} className="alerts-item">
            <div className="alerts-item-main" style={{ flex: 1 }}>
              ...
              <div className="meta" style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginTop: 4 }}>
                <select
                  ...
                  onChange={(e) => void update(search.id, { frequency: e.target.value })}
                >
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 6 }}>
                <label className="toggle-control toggle-control-sm">
                  <input
                    ...
                    onChange={(e) => void update(search.id, { notifyReminders: e.target.checked })}
                  />
                  <span className="toggle-control-track" aria-hidden />
                ...
                <label className="toggle-control toggle-control-sm">
                  <input
                    ...
                    onChange={(e) => void update(search.id, { notifyUpdates: e.target.checked })}
                  />
                  <span className="toggle-control-track" aria-hidden />
            ...
            <div className="alerts-item-actions" style={{ display: "flex", gap: 6, flexDirection: "column" }}>
              <button className="secondary-btn" type="button" onClick={() => void update(search.id, { paused: !search.unsubscribedAt })}>
                {search.unsubscribedAt ? t("profile.savedSearches.resume") : t("profile.savedSearches.pause")}
              </button>
              <button className="secondary-btn" type="button" onClick={() => void remove(search.id)}>
